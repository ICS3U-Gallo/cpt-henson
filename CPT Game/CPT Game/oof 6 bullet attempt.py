...
class MyGame(arcade.Window):
    ...
    def update(self, delta_time):
        ...
        global JUMP_CHECK, JUMP_CLOCK, fire_check, firerate_clock, SPREAD
        ...
        if loadout == 1:
            BULLET_SPEED = 10
            FIRERATE = 14
            BULLET_SPRITE = arcade.Sprite("images/bullet.png", 2)
            SPREAD = 0

        if loadout == 2:
            BULLET_SPEED = 20
            FIRERATE = 32
            BULLET_SPRITE = arcade.Sprite("images/bullet_red.png", 3)
            SPREAD = 0

        if loadout == 3:
            BULLET_SPEED = 8
            FIRERATE = 5
            BULLET_SPRITE = arcade.Sprite("images/bullet_yellow.png", 2)
            SPREAD = random.uniform(-1, 1)

        if self.p_pressed == True:


            if firerate_clock == 0 and fire_check == True:
